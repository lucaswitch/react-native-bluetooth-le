...
import {ButtonGroup, Container, ListItem} from "./style";
...
export function Discovery() {
    ...
    const renderItem = useCallback((props: { index: number, item: BluetoothDevice }) => {
        const {address, name, rssi, dbm} = props.item;
        return <ListItem key={props.index}>
            <OverlineText style={{color: '#000'}}>
                &#x25CB;{' '}{(name) ? name : 'No name device'}{(address ? `, ${address}` : '')}
            </OverlineText>
            <OverlineText>
                {' | '}{rssi}dB, {dbm}dBm
            </OverlineText>
